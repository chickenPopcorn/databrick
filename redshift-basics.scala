...
val tempDir = "s3n://spark-redshift-temp-table/temp/"
...
val awsAccessKeyId = dbutils.secrets.get(scope = "global", key = "awsAccessKeyId")
val awsSecretAccessKey = dbutils.secrets.get(scope = "global", key = "awsSecretAccessKey")

// // Set your s3a and/or s3n keys depending on the temp directory set above
...
val jdbcUsername = dbutils.secrets.get(scope = "global", key = "jdbcUsername")
val jdbcPassword = dbutils.secrets.get(scope = "global", key = "jdbcPassword")
val jdbcHostname = dbutils.secrets.get(scope = "global", key = "jdbcHostname")
val jdbcPort = dbutils.secrets.get(scope = "global", key = "jdbcPort")
val jdbcDatabase = dbutils.secrets.get(scope = "global", key = "jdbcDatabase")
...
someDF
  ...
  .option("dbtable", "diamonds") // <--- name of the table to create in Redshift
...
sqlContext.sql("select distinct * from diamonds")
  .write
  .format("com.databricks.spark.redshift")
  .option("url", jdbcUrl)
  .option("tempdir", tempDir)
...
val diamonds_from_redshift = sqlContext.read
  ...
  .option("forward_spark_s3_credentials", true)
  .load()
diamonds_from_redshift.show

// COMMAND ----------

diamonds_from_redshift.createOrReplaceTempView("diamonds")
...
diamonds_from_redshift.printSchema()
...
val fromRedshiftQuery = sqlContext.read
  .format("com.databricks.spark.redshift")
  .option("url", jdbcUrl)
  .option("tempdir", tempDir)
  .option("query", "SELECT * from diamonds")
  .option("forward_spark_s3_credentials", true)
  .load()

// COMMAND ----------

display(fromRedshiftQuery)
...
var diamondsDf = sqlContext.sql("select * from diamonds").withColumnRenamed("table", "_table")

// Apply each column metadata customization
columnLengthMap.foreach { case (colName, length) =>
  val metadata = new MetadataBuilder().putLong("maxlength", length).build()
  diamondsDf = diamondsDf.withColumn(colName, diamondsDf(colName).as(colName, metadata))
}
...
diamondsDf.write
  ...
  .option("url", jdbcUrl)
  .option("tempdir", tempDir)
  .option("dbtable", "diamonds_optimized")
  .save()
...
// MAGIC `spark-redshift` executes a Redshift 
// MAGIC 1. [UNLOAD](http://docs.aws.amazon.com/redshift/latest/dg/r_UNLOAD.html) command (using JDBC) which copies the Redshift table in parallel to a temporary S3 bucket provided by the user. 
// MAGIC 2. Next it reads these S3 files in parallel using the Hadoop InputFormat API and maps it to an RDD instance. 
// MAGIC 3. Finally, it applies the schema of the table (or query), retrieved using JDBC metadata retrieval capabilities, to the RDD generated in the prior step to create a DataFrame instance.
// MAGIC 
...
// MAGIC 1. `spark-redshift` will first create the table in Redshift using JDBC. 
// MAGIC 2. It then copies the partitioned RDD encapsulated by the source DataFrame instance to the temporary S3 folder. 
// MAGIC 3. Finally, it executes the Redshift [COPY](http://docs.aws.amazon.com/redshift/latest/dg/r_COPY.html) command that performs a high performance distributed copy of S3 folder contents to the newly created Redshift table.
// MAGIC 
